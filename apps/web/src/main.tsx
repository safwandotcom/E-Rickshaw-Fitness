...
function App() {
  ...
  return <main>
    ...
    {view === 'inspection' ? <section><section className="verification-panel"><h2>{t('vehicleRegistryHeading')}</h2><p className="muted">{t('vehicleRegistryHelp')}</p><div className="inline-form"><label>{t('chassisNumberLabel')}<input value={vehicleSearch} onChange={(event) => setVehicleSearch(event.target.value)} placeholder="e.g. ER-8821" /></label><button type="button" onClick={() => void searchVehicle()} disabled={vehicleLookupLoading}>{vehicleLookupLoading ? t('searching') : t('searchVehicle')}</button></div>{vehicleResult ? <p className="result valid">{t('foundLabel')} <strong>{vehicleResult.chassis_number}</strong> · UUID <code>{vehicleResult.id}</code> · {t('motorLabel')} {vehicleResult.motor_number ?? '—'} · {t('statusLabel')} {vehicleResult.status}</p> : null}<details><summary>{t('registerNewVehicle')}</summary><form onSubmit={(event) => { event.preventDefault(); void registerVehicle(event.currentTarget); }}><label>{t('chassisNumberLabel')}<input name="chassis_number" required /></label><label>{t('motorNumberLabel')}<input name="motor_number" /></label><label>{t('ownerPhoneLabel')}<input name="owner_phone" required placeholder="01XXXXXXXXX" /></label><label>{t('districtUuidLabel')}<input name="district_id" required /></label><label>{t('zoneUuidLabel')}<input name="zone_id" required /></label><button type="submit">{t('registerVehicleButton')}</button></form></details></section><h2>{t('submitInspectionHeading')}</h2><form onSubmit={(event) => { event.preventDefault(); void submitInspection(event.currentTarget); }}>
      ...
      <button type="submit">{t('submitOrSaveOffline')}</button><button type="button" onClick={() => void syncDrafts()}>{t('syncSavedInspections')}</button>
    </form></section> : view === 'verification' ? <section><h2>{t('certificateVerificationHeading')}</h2><p className="muted">{t('verificationHelp')}</p><div className="verification-panel"><h3>{t('offlineQrSignatureHeading')}</h3><QrCameraScanner onDecode={(text) => { setCertificate(text); void verifyCertificate(text); }} /><label>{t('qrPayloadLabel')}<textarea value={certificate} onChange={(event) => setCertificate(event.target.value)} placeholder="ERF1...." /></label><button onClick={() => void verifyCertificate()}>{t('verifySignatureOffline')}</button><p className="result" role="status" aria-live="polite">{verification || t('noOfflineVerificationYet')}</p></div><div className="verification-panel"><h3>{t('liveCertificateStatusHeading')}</h3><label>{t('certificateShortCodeLabel')}<input value={shortCode} onChange={(event) => setShortCode(event.target.value)} placeholder="e.g. X9K2L" /></label><button onClick={() => void lookupShortCode()} disabled={liveVerificationLoading}>{liveVerificationLoading ? t('checking') : t('checkLiveStatus')}</button>{liveVerification ? <p className={`result ${liveVerification.valid ? 'valid' : 'invalid'}`} role="status" aria-live="polite">{liveVerification.valid ? `Active certificate ${liveVerification.certificate_number ?? ''}. Zone: ${liveVerification.zone ?? '—'}. Chassis: ••••${liveVerification.chassis_suffix ?? '—'}. Expires: ${liveVerification.expires_at ? new Date(liveVerification.expires_at).toLocaleDateString() : '—'}.` : `Live status: ${liveVerification.status ?? 'not valid'}. ${liveVerification.reason ?? 'Certificate is inactive, expired, or not found.'}`}</p> : <p className="muted">{t('noLiveLookupYet')}</p>}</div></section> : <>
      ...
      <section><div className="section-heading"><div><h2>{t('operationsSummaryHeading')}</h2><p className="muted">{t('operationsSummaryHelp')}</p></div><button type="button" onClick={() => void loadSummary()} disabled={summaryLoading}>{summaryLoading ? t('refreshing') : t('refreshSummary')}</button></div>{summary ? <div className="summary-grid">{([['rickshaws', t('summaryRickshaws')], ['inspections', t('summaryInspections')], ['paid_bills', t('summaryPaidBills')], ['active_certificates', t('summaryActiveCertificates')], ['queued_notifications', t('summaryQueuedNotifications')]] as Array<[keyof Summary, string]>).map(([key, label]) => <div className="summary-card" key={key}><span>{label}</span><strong>{summary[key].toLocaleString()}</strong></div>)}</div> : <p className="muted">{t('summaryTokenHelp')}</p>}</section>
      <section><div className="section-heading"><div><h2>{t('reconciliationHeading')}</h2><p className="muted">{t('reconciliationHelp')}</p></div><button type="button" onClick={() => void loadReconciliation()} disabled={reconciliationLoading}>{reconciliationLoading ? t('refreshing') : t('refreshExceptions')}</button></div>{reconciliation ? (reconciliation.length > 0 ? <table className="reconciliation-table"><caption className="visually-hidden">{t('reconciliationHeading')}</caption><thead><tr><th scope="col">{t('tableBill')}</th><th scope="col">{t('tableBillStatus')}</th><th scope="col">{t('tableAmount')}</th><th scope="col">{t('tableProvider')}</th><th scope="col">{t('tablePaymentStatus')}</th><th scope="col">{t('tablePaymentAmount')}</th><th scope="col">{t('tableWhen')}</th></tr></thead><tbody>{reconciliation.map((row) => <tr key={`${row.bill_code}-${row.provider_transaction_id ?? 'none'}`}><td>{row.bill_code}</td><td>{row.bill_status}</td><td>{(row.bill_amount_paisa / 100).toFixed(2)} BDT</td><td>{row.provider ?? '—'}</td><td>{row.payment_status ?? '—'}</td><td>{row.payment_amount_paisa !== null ? `${(row.payment_amount_paisa / 100).toFixed(2)} BDT` : '—'}</td><td>{row.payment_at ? new Date(row.payment_at).toLocaleString() : '—'}</td></tr>)}</tbody></table> : <p className="muted">{t('noReconciliationExceptions')}</p>) : <p className="muted">{t('reconciliationTokenHelp')}</p>}</section>
    </>}
    </div>
  </main>;
}
