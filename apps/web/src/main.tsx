...
function App() {
  ...
  return <main>
    ...
    </form></section> : view === 'verification' ? <section><h2>Certificate verification</h2><p className="muted">Offline signature verification works without internet. Live lookup checks the current certificate status and revocation state.</p><div className="verification-panel"><h3>Offline QR signature</h3><QrCameraScanner onDecode={(text) => { setCertificate(text); void verifyCertificate(text); }} /><label>QR payload<textarea value={certificate} onChange={(event) => setCertificate(event.target.value)} placeholder="ERF1...." /></label><button onClick={() => void verifyCertificate()}>Verify signature offline</button><p className="result">{verification || 'No offline verification performed yet.'}</p></div><div className="verification-panel"><h3>Live certificate status</h3><label>Certificate short code<input value={shortCode} onChange={(event) => setShortCode(event.target.value)} placeholder="e.g. X9K2L" /></label><button onClick={() => void lookupShortCode()} disabled={liveVerificationLoading}>{liveVerificationLoading ? 'Checking…' : 'Check live status'}</button>{liveVerification ? <p className={`result ${liveVerification.valid ? 'valid' : 'invalid'}`}>{liveVerification.valid ? `Active certificate ${liveVerification.certificate_number ?? ''}. Zone: ${liveVerification.zone ?? '—'}. Chassis: ••••${liveVerification.chassis_suffix ?? '—'}. Expires: ${liveVerification.expires_at ? new Date(liveVerification.expires_at).toLocaleDateString() : '—'}.` : `Live status: ${liveVerification.status ?? 'not valid'}. ${liveVerification.reason ?? 'Certificate is inactive, expired, or not found.'}`}</p> : <p className="muted">No live lookup performed yet.</p>}</div></section> : <>
